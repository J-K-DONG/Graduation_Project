...
import ImageWeightMatrix as ImageWeightMatrix
import datetime


class ImageFusion(Utility.Method):

    def __init__(self, sf_gpu_available=False):
    ...
    def get_spatial_frequency_matrix(self, images):
        """
        选择计算权值矩阵的方法(GPU mode or CPU mode)
        :param images:
        :return:
        """
        matrix = ImageWeightMatrix.ImageWeightMatrix()
        # matrix_method = "matrix_by_for"
        # matrix_method = "matrix_by_numpy"
        # matrix_method = "matrix_by_signal"
        # matrix_method = "matrix_by_pytorch"
        # print(matrix_method)
        # if matrix_method == "matrix_by_for":
        #     weight_matrix = matrix.get_spatial_frequency_matrix_by_for(images, self.block_size)
        # elif matrix_method == "matrix_by_numpy":
        #     weight_matrix = matrix.get_spatial_frequency_matrix_by_numpy(images, self.block_size)
        # elif matrix_method == "matrix_by_signal":
        #     weight_matrix = matrix.get_spatial_frequency_matrix_by_signal(images, self.block_size)
        # elif matrix_method == "matrix_by_pytorch":
        #     weight_matrix = matrix.get_spatial_frequency_matrix_by_pytorch(images, self.block_size)

        if self.sf_gpu_available:
            matrix_method = "matrix_by_pytorch"
            weight_matrix = matrix.get_spatial_frequency_matrix_by_pytorch(images)
        else:
            # matrix_method = "matrix_by_for"
            # weight_matrix = matrix.get_spatial_frequency_matrix_by_for(images, self.block_size)
            ...
            # matrix_method = "matrix_by_singal"
            # weight_matrix = matrix.get_spatial_frequency_matrix_by_signal(images)

        print("the method of calculating matrix is : ", matrix_method)
